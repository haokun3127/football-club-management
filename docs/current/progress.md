...
- 通过项目内置 WeChatIDE MCP 客户端重新接入已启动的微信开发者工具，取得两张严格 `375×812` 真实模拟器截图：P2 训练详情和 P2.1 比赛详情；sidecar 均确认 `captureMethod=wechatide-mcp simulator_screenshot`。
...
- P6 的指标名称、分数、趋势记录、教练评语、同队均值和日期保留真实接口数据，不复制 Figma 示例；Hero、趋势图、教练评语、同队对比和家长 TabBar 结构通过。
- P7 `93:336` 已重新读取当前在线 Figma 上下文和截图，并通过 WeChatIDE MCP 取得真实家长路由 `/pages/parent/child/index` 的严格 `375×812` 运行图。顶栏、深色学员卡、双角色切换入口、统计卡、四个快捷操作、最近动态和家长 TabBar 结构通过，无需代码修复。
- P7 运行态的学员、训练课时、出勤率和动态来自真实接口，与 Figma 静态示例不同；证据和并排对照图位于 `%TEMP%\cq-talent-visual-evidence`。
...
# 2026-08-31 P9 私教预约真实 MCP 复验与修复

- 在线 Figma `zZ6wKyOHKcO4UYXDd9jGwv / 93:500` 已重新读取，并通过真实微信开发者工具 MCP 对 `/pages/parent/private/index` 取得严格 `375×812` 运行截图。
...
## 2026-09-03 家长成长足迹真实时间线

- `growth-summary` 新增服务端 `timeline` 投影：只返回当前家长有权查看的学员事实。完成训练包含真实训练内容、课堂评分与备注；完成比赛包含对手、比分、场地及该学员自己的比赛事件；能力更新来自课堂训练评分或绑定学期任务的真实能力记录。
...
## 2026-09-03 浏览器 Figma 施工入口

- 浏览器编辑会话已验证可写，并非破坏性克隆 C2 工作台为新节点 `1955:7`（`375×812`，位置 `x=1285,y=610`）；原 C2 `1610:1462` 未被覆盖。MCP 重新读取已确认该节点存在。
- 新节点只作为 C2.1 课堂训练评测的施工入口，尚未完成评测页面的视觉重组，不作为视觉验收或需求完成证据。当前代码/API 测试与 GitHub 发布状态不受此设计在途工作影响。

## 2026-09-03 C2.1 课堂训练评测在线稿与实现同步

- 在线 Figma C2.1 `1955:7` 已在原 C2 `1610:1462` 的非破坏性副本内完成首屏重组：全屏返回导航“课堂训练评测”、右上保存、活动日期/球队/时间/场地/已到人数、训练内容切换、已到学员评分与备注语义、底部“保存本堂课堂评测”；子页面不再带 Coach TabBar 或旧出勤头像宫格。
...
## 2026-09-03 训练评测与成长足迹目标完成审计

- 训练内容评测：API 回归证明只允许“训练活动 × 已选训练内容 × 已点名到场学员”写入，且课堂评分通过文件 SQLite 重启后可读回。
- 学期测评：API 回归证明任务按球队、学期和模板范围创建／读取，提交携带任务 ID；持久化回归证明任务和任务绑定的评估记录重启后仍可读回。
- 家长成长：API 回归证明课时统计只计算训练的已到／应到，成长时间线只返回当前孩子可见的真实训练、比赛和能力更新；P4 与成长足迹全屏页都由同一服务端时间线投影渲染。
- 本轮新鲜验证：目标 API 回归 `4/4`、目标 SQLite 重启回归 `3/3`、关联小程序页面回归 `63/63`、小程序 TypeScript 和 `git diff --check` 通过。工作树干净，`dev` 远端与本地提交一致。全仓门禁的两项既有小程序阻断继续按上一条记录处理，未将其误记为本目标回归。

## 2026-09-03 成长历程课时累计 V7 第一批

- 在线 Figma 已非破坏性新增 Parent V7：P4 成长入口 `1967:2`、能力模型历史 `1967:164`、比赛详情 `1967:198`；Coach V7 同步新增测评任务 `1973:2`、项目选择 `1973:91`、全队批量录入 `1973:131`、能力评估 `1973:181` 与学员雷达 `1973:302`。每个节点均经 Figma MCP 截图回读；比赛详情的比分与队名碰撞已在写稿后修正并重新回读。
- 本批已实现“训练历程的每堂已完成训练显示当时累计课时”：服务端时间线按完成训练的时间顺序计算真实 `attendedLessons / expectedLessons`，小程序训练历程直接消费同一成长时间线并展示如 `18/22课时`，不再扫描日历后在客户端推算。
- 定向验证：成长时间线 API 回归 `1/1`、训练历程页面回归 `3/3`、小程序 TypeScript 与限定路径 `git diff --check` 通过。尚未声称新鲜 DevTools/真机视觉验收。

## 2026-09-03 能力模型历史页第一批

- 按在线 Figma Parent V7 `1967:164` 完成家长端全屏“能力模型更新”页：摘要区、课堂训练／学期测评来源标签、更新时间、指标前后值、变化值和来源跳转均由真实成长时间线预计算。
- 成长足迹里的“能力更新”记录新增“查看更新”入口，跳转到独立历史页；训练来源回到活动详情，学期测评来源进入对应指标详情。未新增伪造会话、角色、接口或数据。
- 验证：能力模型历史与成长足迹定向 Vitest `5/5`、小程序 TypeScript `--noEmit`、限定路径 `git diff --check` 通过。尚未取得新的微信开发者工具运行态截图，不能记为真机视觉验收。

## 2026-09-03 比赛详情 V7 第一批

- 按在线 Figma Parent V7 `1967:198` 调整家长端比赛详情：日期/时间置于顶栏下，白色比分卡展示真实场地、双方、比分和结果；比赛事件改为当前孩子的事件列表，并保留真实教练点评（缺失时不展示空文案）。
- 比赛详情不再显示家长 TabBar 或无关的出席名单；训练与其他活动详情仍沿用原有页面分支。比赛历史卡继续通过真实活动 ID 跳转到 `/pages/parent/event/index`。
- 验证：活动详情、比赛记录、成长足迹、能力历史联合 Vitest `16/16`，小程序 TypeScript `--noEmit` 与 `git diff --check` 通过。尚未取得新的微信开发者工具运行态截图，不能记为运行态视觉验收。

## 2026-09-03 C11 测评任务入口文案同步

- 按在线 Figma Coach V7 `1973:2` 将顶栏创建入口从“新增”统一为“新建任务”，保留顶栏和红色悬浮按钮两个真实导航入口；球队/学期、进度和状态仍由测评任务 API 提供。
- 验证：C11 定向 Vitest `10/10`、小程序 TypeScript `--noEmit` 与 `git diff --check` 通过。尚未取得新的微信开发者工具运行态截图，不能记为运行态视觉验收。

## 2026-09-03 C12 测评项目选择第一批

- 按在线 Figma Coach V7 C12 `1973:91` 与 C12.1 `1973:131` 增加全屏测评项目选择页。任务列表进入后先校验真实任务/模板状态，再从真实测评表单的 `groupId/groupLabel` 分组项目；项目进度接口尚未提供时显示“待录入”，不虚构已录人数。
- 选中项目后携带 `taskId + templateId + projectId` 进入现有能力录入页，录入页只加载该项目对应的真实指标；后续全队连续录入和按项目草稿保存仍是下一批工作。
- 验证：C11、C12 项目选择和 C15 原录入联合 Vitest `22/22`、小程序 TypeScript `--noEmit` 与 `git diff --check` 通过。尚未取得新的微信开发者工具运行态截图，不能记为运行态视觉验收。

## 2026-09-03 C12.1 全队批量测评录入

- 沿用在线 Figma Coach V7 C12.1 `1973:131` 的全屏项目录入层级：任务先选项目，再按项目展示全队学员；底部固定“保存本项目 / 下一项目”操作区避开安全区，不显示角色 TabBar。
- 新增 `/pages/coach/assessment-bulk-entry/index`：校验真实进行中的测评任务、模板和球队，按项目分组字段，展示所有真实队员和原始成绩输入；所有 WXML 展示字段均在 TS 预计算。
...
## 2026-09-03 C12.1 已完成测评任务回看收口

- 设计先行：在线 Figma `zZ6wKyOHKcO4UYXDd9jGwv` 已新增非破坏性完成态只读画板 `1985:2`（`C12.1 · Team Batch Entry · V7 · Completed Read-only`），并已截图回读；旧画板未移动或删除。
- 真实生产测评任务在 19 名队员全部提交后，状态会合法变为 `completed`。此前 C11/C12/C12.1 只接受 `in_progress`，导致刚保存完成的真实成绩无法被教练再次打开；这是状态机边界问题，不是数据丢失。
- 小程序现允许从任务列表和项目列表进入 `completed` 任务：C12.1 会读回已保存的原始成绩与标准分，显示“已完成 · 只读”，禁用修改与保存，并通过“返回项目”回到项目选择。`not_started` 仍不能进入。
- 客户端任务列表新增强制刷新选项，以唯一 `refresh=<timestamp>-<sequence>` 查询参数绕过微信开发者工具的短缓存，避免保存后的列表仍显示旧状态；没有增加伪 API、伪会话或客户端假数据。
- 新鲜验证：小程序定向 Vitest `37/37`、`tsc --noEmit` 与 `git diff --check` 均通过。DevTools 本轮导航仍会回落到教练日程，因此尚未把任何截图误记为 C12.1 运行态视觉验收。

## 2026-09-03 C12.1 完成态行状态与真实回填复拍

- Figma 完成态画板 `1985:2` 先行修正：每个队员行的状态明确为“已保存”，并重新回读原生 `375×812` 截图。
...
## 2026-09-03 P4 成长足迹入口文案对齐

- 重新读取在线 Figma Parent V7 P4 `1967:2`，确认成长足迹卡片右上角文案为“查看全部 ›”。
- 先新增并运行回归测试，准确复现旧实现“更多›”不符合设计稿；随后仅修改 P4 WXML 为“查看全部 ›”，没有改变导航逻辑。
- 验证：家长成长页 Vitest `9/9`、小程序 TypeScript 通过、`git diff --check` 通过。

## 2026-09-03 P4.2 比赛详情事件布局恢复

- 重新读取在线 Figma Parent V7 P4.2 `1967:198`，确认比赛详情的比赛数据标题独立于白色事件卡，事件行按“图标 + 事件名称 + 第 X 分钟”排列。
- 先新增回归并准确复现旧实现问题：参与者姓名为通用“学员”时，页面标题无法使用同一比赛事件中真实的球员姓名；随后改为从匹配的 API 事件读取真实姓名，不写死示例数据。
- 清除旧 WXSS 末尾 `.match-event-row__name { display: none; }` 覆盖规则；它是结构改版后运行态只显示图标和分钟、事件名称消失的根因。
- 验证：家长成长/足迹/能力历史/比赛记录/比赛详情/训练历程联合 Vitest `29/29`，小程序 TypeScript 通过，`git diff --check` 通过。
- 运行态复拍证据：`C:\Users\ASUS\AppData\Local\Temp\cq-talent-p42-detail-runtime-final-20260903.png`，尺寸 `390×844`；真实页面显示“丁宁的比赛数据”和 5 条真实比赛事件的图标、事件名、分钟。该图只记为运行态证据，不冒充严格 `375×812` 视觉验收。

## 2026-09-03 C12.1 完成态展示对齐

- 先重新读取在线 Figma 完成态画板 `1985:2`，确认项目标题不带“录入”，摘要为“全队 N 人已完成 · 成绩已保存”，队员行展示原始值/单位和只读“已保存”，不再使用禁用输入框作为完成态视觉。
- C12.1 现在按任务状态分支：进行中的项目继续使用输入框和草稿；已完成项目使用真实 API 回填的原始值与单位，只读展示并固定显示“已保存”。没有增加示例分数或伪数据。
- 完成摘要由真实球队成员数量与已回填成绩计算，进行中任务原有保存、失败重试和草稿隔离逻辑不变。
- 验证：C12.1 定向 Vitest `7/7`、小程序 TypeScript 通过、`git diff --check` 通过；等待刷新开发者工具后复拍完成态运行证据。

## 2026-09-03 C12.1 完成态只读下划线清理
...
## 2026-09-04 C12 课堂训练评测入口提示与缺少训练内容处理

- 用户反馈课堂训练评测进入后显示通用“读取失败”，同时 C12 项目选择页不清楚从哪里开始录入。
- 根因：本堂训练尚未在训练内容页选择项目时，后端按契约返回 `400 training_content_required`；小程序此前把该业务状态统一吞成错误态。C12 的进度卡也只有“先选择测评项目”标题，未明确指向下方可点击的项目卡。
- 小程序现在识别 `training_content_required`，显示“暂无训练内容”和“去选择训练内容”，按钮直接跳转当前活动的训练内容选择页；其它网络/权限错误仍保留重试态。
- C12 进度卡补充“请选择下方测评项目开始录入”，实际项目卡仍按真实模板指标分组并可点击进入批量录入。
- 验证：课堂评测与 C12 定向 Vitest `10/10`，小程序全量 Vitest `499/499`，TypeScript 通过，`git diff --check` 通过。

## 2026-09-04 C11 悬浮新增按钮层级对齐

- 重新读取在线 Figma Coach C11 `zZ6wKyOHKcO4UYXDd9jGwv / 93:1002`，确认悬浮新增按钮仍位于右侧 `28px`、底部 `56px`，但底部 TabBar 覆盖按钮下沿。
- 小程序保留按钮几何位置，将 `.tasks-fab` 的层级从 `10000` 调整为 `9998`，低于 `role-tabbar` 的 `9999`；避免整颗按钮压在 TabBar 上方，与在线稿的遮挡关系一致。
- 验证：C11 Vitest `10/10`、小程序 TypeScript、WXSS 编译及 `git diff --check` 通过；WeChatIDE MCP 实际截图严格 `375×812`，按钮左上角为 `(291,700)`，控制台错误过滤无命中。

## 2026-09-04 C12 测评项目选择复用训练内容交互

- 用户确认测评项目必须与训练内容选择保持一致：项目卡可多选，卡片显示项目图标、指标数量、录入进度和选中态，底部固定栏显示已选数量，确认后才进入评分。
- 小程序 C12 从单选圆圈改为与训练内容选择页一致的多选项目列表；确认时把本次选中的项目集合传给批量录入页，批量录入的“下一项目”只遍历本次选择的项目，不再把未选择项目混入流程。
- 保留真实模板字段分组、真实任务进度和已保存记录；未新增示例分数或伪数据。主工作区 WeChatIDE MCP 真实截图：`C:\Users\ASUS\Desktop\cq-talent-assessment-project-select-v2.png`、`C:\Users\ASUS\Desktop\cq-talent-assessment-project-selected-v2.png`，均为 `375×812`。
- 验证：C12 项目选择 + C12 批量录入 Vitest `12/12`，小程序 TypeScript、C12 WXML/WXSS 编译、`git diff --check` 通过。

## 2026-09-04 C12 测评项目三级选择页收口

- 测评项目选择页现在复用训练内容选择页的三级结构：一级能力域横向切换、二级指标侧栏、三级测评项目分组；项目卡支持多选，底部固定确认栏统一进入批量评分。
- 测评项目按表单字段真实 `metricId` 映射到能力树三级节点；未能匹配能力树的真实模板项目进入独立的“其他”一级入口和“其他测评项目”二级/三级分组，不会挂到任意无关能力域下，也不把评分表 Excel 硬编码成前端数据。
- 增加一级/二级切换回归覆盖，保留任务球队、学期、真实录入进度和已保存人数；WXML 展示字段继续由 TS 预计算，没有使用数组方法调用。
- 微信开发者工具 MCP 主工作区截图：`C:\Users\ASUS\Desktop\cq-talent-assessment-project-select-v5.png`，`375×812`；通过真实任务 `assessment-task-app-client-4aeb45d0-5dba-4d1d-8e33-1381a6ff8e2b` 验证无匹配项目会默认进入独立“其他”入口，且项目卡仍可选择。该任务的 `metric-finishing` 不在当前能力树中，因此运行态显示“其他测评项目”，属于真实数据映射结果，不是伪造分类。
- 验证：C12 项目选择 Vitest `4/4`，C2/C11/C12/C15/训练评测相关联合 Vitest `35/35`，小程序 TypeScript、WXML/WXSS 编译和 `git diff --check` 通过；随后通过本地 `pnpm.cmd run check` 完成全仓门禁：domain `21/21`、小程序 `499/499`、API `130/130`。当前没有进行在线 Figma 写入，本批只同步已有训练内容选择交互到代码。

## 2026-09-04 C12 测评项目空分类修复

- 用户反馈 C12 切换能力域后几乎全部显示“当前分类暂无测评项目”。根因是页面只按 `metricId` 匹配训练能力树；真实任务模板的输入指标 `metric-finishing` 不在当前训练树中，旧逻辑因此把项目放入通用“其他”入口，原有能力域则全部成为空分类。
- 页面现改为：先按真实 `metricId` 匹配；匹配不到时按测评表真实维度名称寻找同名一级能力域，找不到才生成以真实维度名称命名的独立一级入口。导航只保留实际包含项目的一级/二级分支，不再展示空能力域，也不把项目挂到无关分类。
- 真实任务验证截图：`C:\Users\ASUS\AppData\Local\Temp\cq-talent-assessment-projects-after-fix.png`，严格 `375×812`；页面实际显示“技术能力 → 测评项目 → 技术能力”及 1 张真实项目卡。当前线上任务模板只有 1 个真实 `input` 绑定，故页面显示 1 张卡；未绑定的测试项目不会被前端擅自补出。
- 验证：C12 定向 Vitest `5/5`，C12/C15/C11 联合 Vitest `23/23`，小程序 TypeScript、微信开发者工具 MCP WXML/WXSS 编译和 `git diff --check` 通过。
...
## 2026-09-04 C12 分级选择逻辑纠正

- 用户确认测评项目选择必须与训练内容选择保持一致的三级分级：一级能力域横向切换、二级指标侧栏、三级测评项目列表。上一版仅保留单列项目列表，误删了分级导航，现已恢复。
- 三级项目卡保留全宽布局，不再使用窄双列；项目名称支持两行，说明、真实录入状态和选择控件清晰展示，选择圆圈固定在卡片右侧。
- 复拍文件为 C:\Users\ASUS\AppData\Local\Temp\cq-talent-assessment-projects-hierarchy.png，尺寸 390×844；截图显示一级“运控球”、二级“球性球感”、三级“基础球性”和项目卡，分级切换可用。
- 验证：C12、批量录入、测评任务和训练内容选择测试 26/26，小程序 TypeScript、WXSS 编译和 git diff --check 通过。

## 2026-09-04 七套受控测试账号滚动状态收口

- 只读生产审计发现，七套受控测试账号虽有完整人员、训练、比赛、测评和战术板数据，但部分历史训练/比赛仍保留 `scheduled`，且历史参与人还处于邀请、确认或请假申请状态；这会让日程和出勤页面展示过期、不一致的状态。
- 受控刷新现在会严格按 `club_id + cq-talent-secure-test 槽位 ID 前缀 + 对应球队或对应测试教练` 限定范围：已结束活动改为完成、未来活动维持待进行；历史参与人的待确认/邀请收口为已到场，请假申请收口为已批准请假；关联比赛状态同步。兼容早期误关联到旧球队、但仍由对应测试教练创建的受控记录；普通俱乐部数据、其他球队、其他教练或其他 ID 命名空间均不触碰。
- 同时修复滚动生成规则：当天较晚运行刷新时，“本周技术训练”自动排到下一个可用训练时段，避免生成后立即成为过期的待进行活动。已补回归测试覆盖未来活动、历史状态收口和跨球队隔离。
- 本地验证：受控测试账号 Vitest `22/22`、API TypeScript `--noEmit` 和 `git diff --check` 通过。生产刷新将在提交、部署、受限备份和 dry-run 后执行，并以聚合审计确认结果。

### 生产执行结果

- 已发布 API `69b4960`；发布过程完成受限 SQLite 备份、镜像构建和容器重建。部署脚本的首次内部轮询因 Node 冷启动过快结束，但随后只读核验确认容器运行、新版本标签、内部健康与 `https://cqtc.pomi.tech/health` 均为正常 `200`。
- 已按受控流程完成 dry-run、刷新前专用 SQLite 备份、confirmed import 与 API 重启；未输出账号号码、会话或运行时密钥。
- 最终只读状态审计：历史未完成活动 `0`、未来非待进行活动 `0`、历史待确认/邀请/待批假出勤 `0`、活动与比赛状态不一致 `0`。七个槽位的聚合审计全部通过，均保留 19 人名单、两名家长绑定学员、最近三周活动、12 条未来活动、中文场地/展示文案、19 条测评、152 条雷达记录、2 场比赛、8 条比赛事件以及 19 人战术板。

## 2026-09-04 C12 项目库与本次测评范围语义修正

- 评分表中的 62 个三级项目是可选项目库，不代表每次测评都必须完成 62 项。
- C12 仍按训练内容同款三级分级选择；教练只勾选本次需要测评的项目，确认后 C12.1 仅录入已选择项目，未选择项目不会进入评分流程。
- 进度卡改为“可选 X 个项目 · 已录入 Y 个项目 · 学员进度”，不再使用“已完成 Y/X 个项目”造成全部项目必测的误解；说明改为“请选择本次需要测评的项目”。
- 已用 390×844 模拟器截图复核 C12；C12 与 C12.1 测试 16/16、TypeScript 检查及 WXML 编译通过。截图仅用于当前模拟器视觉复核，不等同于 375×812 真机验收。

## 2026-09-04 C14 学员头像颜色统一

- 用户反馈能力评估页的首位学员头像为红色、其他头像为蓝色。根因是 `.ability-player__avatar` 将默认背景设为蓝色 `#3971ba`，并另以 `:first-child` 覆盖首位为正红，属于按列表顺序产生的错误视觉差异，不涉及球员数据或状态。
- 设计先行：在线 Figma C14 V7 `1973:181` 的可见学员头像已统一为天才体育正红 `#A80F1B`，回读截图为原生 `375×812`。
- 小程序删除首位选择器和蓝色默认值，所有 C14 学员头像共用 `#a80f1b`；回归测试明确禁止恢复首位特例或蓝色值。
- 验证：C14 Vitest `5/5`、小程序 TypeScript 及 WeChatIDE MCP WXSS 编译通过。MCP 运行态截图在页面数据加载阶段超时，未将其误记为运行态视觉通过。

## 2026-09-05 C14 能力评估加载性能收口

- 生产日志定位到能力评估页原先在读取球队名单后，会为每名学员请求一次 `/coach/students/:id/radar`；19 名学员在微信运行时实际顺序发出，形成约 19 次网络往返。服务器健康检查、CPU 与内存均正常，慢点是前端 N+1 请求模式，不是服务器资源不足。
- `/coach/team/ability-overview` 现在一次返回球队核心雷达维度、球队综合分和每名学员的综合分；数据范围继续受教练球队权限限制，并与个人雷达页使用相同核心能力维度。
- C14 同时发起球队与聚合概览读取：球队名单先渲染，聚合分数到达后再回填。若聚合请求临时失败，名单和球员详情入口仍可使用，分数保留“—”，不会让整个页面停在加载状态；页面不再在列表加载阶段请求每名球员雷达。
- 验证：API + C14 定向 Vitest `64/64`，API 与小程序 TypeScript `--noEmit` 均通过，`git diff --check` 通过。部署后需在真实教练会话重新打开“能力评估”页，确认网络日志仅有球队与能力概览两条业务读取，不再有 19 条球员雷达读取。

### 生产发布结果

- 已发布 GitHub `dev` 提交 `16d3824`。生产发布前完成受限 SQLite 快照，容器镜像与运行标签均为 `16d3824`。
- 部署脚本在 Node 冷启动阶段因连续、无等待的 18 次内部健康轮询提前报错；未回滚、未重置数据库。随后只读复核确认 API 容器正常运行，`http://127.0.0.1:3000/health` 与 `https://cqtc.pomi.tech/health` 均返回 `200`。
- 小程序端代码已在提交中；需要在微信开发者工具点击一次“编译”，再重新打开教练端“能力评估”以加载聚合请求版本。真实会话网络复核仍待该客户端编译后进行。

## 2026-09-05 受控测试日程历史重复训练清理

- 只读生产巡检发现，部分受控测试账号同一时段出现“本周技术训练”与历史 `daily-*` 综合训练两条记录；当前滚动生成器没有产生该冲突，根因是早期注入的 `event-cq-talent-secure-test-<slot>-daily-*` 遗留数据不在后续刷新更新范围内。
- 受控导入器现将该精确 ID 命名空间视为陈旧状态，并只在同一俱乐部、同一受控槽位且归属同一球队或同一测试教练时，删除重复活动及其事件关联数据（出勤、训练/测评、课时、战术、比赛关联数据）。同前缀但属于其他球队或其他测试槽位的记录不会被修改。
- 新增 SQLite 回归：先植入一条同队重叠 `daily-*` 训练及参与记录、再植入同前缀的无关队伍训练；刷新必须返回 `refreshed`，仅删除前者并保留标准日程和无关队伍记录。
- 本地验证：受控账号 Vitest `23/23`、API TypeScript `--noEmit`、`git diff --check` 通过。生产执行待本提交发布、受限 SQLite 备份与受控刷新后，以只读审计确认日程不存在重叠。

### 生产执行结果

- 已发布 GitHub `dev` 提交 `6034ed4`。发布前完成受限 SQLite 快照；容器镜像与运行标签均为 `6034ed4`。部署脚本因旧的无等待健康轮询过早结束，但只读复核确认容器正常运行，内部健康与 `https://cqtc.pomi.tech/health` 均返回 `200`。
- 已完成受控刷新前 dry-run、专用 SQLite 备份、七账号 confirmed import 与 API 重启；运行时账号号码、会话和密钥均未输出。
- 最终只读生产审计：7 个槽位全部 `legacyDailyActivities=0`、`overlappingActivities=0`；同时保留双角色身份、19 人球队、2 名家长绑定学员、三周滚动训练履历、中文场地/展示文案、19 条测评、152 条雷达记录、2 场比赛、8 条比赛事件与 19 人战术板。不同槽位的历史活动数量来自既有已完成训练履历，未发现同队时间冲突。

### 完整功能自检

- API 全量 Vitest 结构化报告：25 个测试文件、129 项测试全部通过；本地门禁同时完成域模型 21 项、小程序 503 项测试和各包 TypeScript 检查。
- 生产数据审计与本地测试分别验证了运行数据和代码契约；两者均通过，但微信真机授权链路与每一页的视觉验收仍应在后续体验版提审前单独执行，不能由自动测试替代。

## 2026-09-05 双端周历点击范围修复

- 根因：家长端 P1 与教练端 C1 都把 `expandMonthPicker` 绑定在整块周历容器；点击星期标签或日期格边缘时，事件会落到外层并错误打开月历。
- 修复：删除两端周历容器的展开绑定，只保留右侧下拉箭头的独立 `catchtap`；日期格继续只执行日期选择，左右箭头继续只切换周。
- 验证：双端日程定向 Vitest `45/45`、小程序全量 Vitest `503/503`、TypeScript、`git diff --check` 通过；微信开发者工具 MCP 已成功编译两份日程 WXML。
