...
- 已消除 ready 状态下 `status-view` 与实际内容同时渲染的宿主问题；C5 的活动元数据分组、名单行高/标签内边距和底部操作区按画板收口。C5.1 将此前容易挤压名单的横向减/数值/加控制收为画板同类的数字框加上下箭头，同时保持每次点击写入真实 `±0.5` 调整值。
- 验证：先更新页面回归并确认失败，后完成最小实现；聚焦 Vitest `15/15`、小程序 typecheck、`git diff --check` 和全仓门禁均通过（domain `19/19`、mini-program `322/322`、API `104/104`）。
- 运行态边界：Automator 成功从 C4 跳到 `/pages/coach/lesson/index`，但嵌入式模拟器截图仍显示上一页 C4.2（未刷新 IDE 编译产物），故没有把该像素图作为 C5/C5.1 视觉验收，也没有据此改动 C5 代码；用户已明确本阶段不把最终截图作为完成前置。
