...
评测模板需要支持指标图谱、依赖边、模板版本、视图展示、测试项目、原始结果、标准化得分和任意指标公式。树状层级只是某个模板或报告的展示视图，不是底层指标关系。第一个俱乐部评分表的具体要求见 [学员能力评估体系分析](../archive/2026-06-discovery/assessment-system-analysis.md)。
...
不同俱乐部的业务差异优先通过功能开关、策略配置、自定义字段和俱乐部级目录承载。详见 [多俱乐部解耦分析](../archive/2026-06-discovery/multi-club-decoupling.md)。
