...
- 生产/release app-client API 不再接受 `X-User-Id` 作为认证；它只可在显式本地开发入口进行隔离 smoke。任何公网验收都必须经真实微信授权和服务端校验后的 Bearer session。
- 最新本地提交 `ddfbc29` 增加了三套隔离双角色测试账号的受控导入/回滚能力，但没有执行服务器访问、数据库备份、导入、部署、重启或真机登录。因此本文件中所有较早的生产演示/账号记录均为历史快照，不能视为当前生产状态；详见 `agent-handover-2026-08-12.md`。

## 2026-08-09 当前发布判定（覆盖后文历史快照）

| 项目 | 当前事实 | 仍需完成 |
| --- | --- | --- |
| 家长端 17 页视觉 | 已完成全页级 Figma↔实页巡检（`miniProgram.screenshot` 375×812 全采+vision 逐页比对）；4 项差异已修复并复验通过（c05e82f/bda7b3b）；批次4/5/6 顶栏与胶囊避让已收口（a360cf5） | 无新增；遗留低优先项见下 |
| 成长页雷达预览 | 已真实数据化（1fe9d8b/e4294d0）：8 轴网格环+真实得分多边形+基准多边形+维度标签，实拍 4/4 通过 | 无 |
| P1 日程 | 巡检未发现新缺陷；08-05 判定表中的 Hero/周序差异为历史低优先遗留，未返工 | 如需对齐 Figma 周序 MON→SUN 再排期 |
| C4 签到 | 同 08-05 判定：部署可达，生产真实 PUT/重启读回/视觉验收仍待完成 | 同左 |
| API 质量 | 根 `pnpm run check` 137 tests 全绿（domain 18/miniprogram 51/api 68）；assessment 持久化贴边慢测试并行负载下偶发 flake，复跑即绿 | 提交在途 assessment 改动时建议 timeout 提至 10s |
