...
- 历史 P5 `93:278` 的新版顶栏曾保留“历史对比”设计预留；当前 Parent V6 P5 `1610:626` 已移除该动作，实际实现不得恢复“历史对比”。
...
## 2026-09-03 P4/P5 视觉删减同步

- P4 课时统计行以当前在线 Figma `1967:21 / Stats Row` 为准：统计行和两条内部竖线均不显示任何描边；小程序对应 `.p4-hero__stats` 只保留布局，不设置外框或内部竖线。
- P5 能力雷达以当前在线节点 `1610:633 / History Compare Button` 为准：节点已隐藏，顶栏只保留返回和“能力雷达”标题；`openMetricHistory` 与 `.p5-nav__compare` 不再属于实现契约。
- 两个节点均已通过 Figma MCP 截图/元数据回读确认；Figma 截图是设计证据，不等同于微信开发者工具运行态视觉验收。
...
## 2026-09-03 训练评测批次 Figma 写入权限核验

- 当前 Figma MCP 身份 `haokun3127` 的团队席位为 `View`。本任务在该通道下只能读取并对照 C2 `1610:1462`、C11 `1617:2`、C15 `1623:2` 与 P4 `1610:466`，不能可靠写入、克隆或重命名在线节点。
- 因此本批新增的课堂训练评测全屏状态、C11/C15 任务范围文案和 P4 成长足迹明细不能被记录为“在线 Figma 已更新”。后续需使用已拥有文件级编辑权的浏览器会话，或将 MCP 账号升级为可编辑席位后，先补齐对应画板、截图回读，再宣称设计稿与实现同步。

## 2026-09-03 浏览器 Figma 会话补充

- 已通过用户已有编辑会话非破坏性克隆 C2 工作台，新增节点 `1955:7`（`C2 · Training Workbench · V6`，`x=1285,y=610`，`375×812`）；原 C2 `1610:1462` 与其他画板均未覆盖或移动。
- 该副本当前仅是课堂训练评测状态页的施工入口，尚未完成训练项目评分、已到学员和保存反馈的完整视觉重组；因此不能将此克隆误记为 C2.1 视觉验收通过。后续设计更新应在此新节点上完成，再回读截图与小程序实现对照。

## 2026-09-03 C2.1 课堂训练评测交付基准

- 当前 C2.1 在线设计基准：`zZ6wKyOHKcO4UYXDd9jGwv / 1955:7`，名称为 `C2.1 · Classroom Assessment · V6`，原生 `375×812`。原 C2 `1610:1462` 继续保留为训练工作台，不得覆盖。
- C2.1 的首屏结构为：固定返回顶栏与保存动作、深色活动上下文卡、横向训练内容选择、已到学员评分卡和底部保存动作；不使用 TabBar，也不把点名头像宫格混入评测页。
- 回读截图：`C:\Users\ASUS\AppData\Local\Temp\cq-talent-c21-figma-assessment-20260903.png`。小程序实现必须从真实活动时间、球队、场地、已到名单和已选训练内容生成视图，不能把画板示例人名或分数写入接口/页面数据。
- 当前 C2.1 已完成在线稿截图回读和代码定向回归；尚无同版本可信微信运行态截图，因此不得把本条写成真机视觉验收通过。

## 2026-09-03 C11/C15/P4 在线稿回读与实现同步

- C11 `1617:2` 的任务卡基准为“标题 / 球队与学期 / 日期 / 进度”；C15 `1623:2` 的上下文为“测评任务”，指标提示使用简短的“输入原始值”；P4 `1610:466` 的课时统计语义为 `已到/应到`，示例比例为 `41/46`。
- 已通过浏览器可编辑会话写入并回读上述在线节点；回读截图分别为 `C:\Users\ASUS\AppData\Local\Temp\cq-talent-figma-audit-20260903\c11-clean.png`、`c15-final.png`、`p4-clean.png`。它们是 Figma 截图，不是微信运行态截图。
- 小程序本批仅同步 C15 的短提示／可收缩布局和 P4 比例数字可读性；不得将此记录理解为三页均已取得新鲜的真机或 DevTools 视觉验收。

## 2026-09-03 Growth / Assessment V7 追加式设计入口

- 当前在线写入已由 Figma MCP 成功回读，仍使用唯一 file key `zZ6wKyOHKcO4UYXDd9jGwv`。V6 与更早画板保留，不覆盖、删除或移动。
- Parent V7：P4 成长入口 `1967:2`；P4.1 能力模型历史 `1967:164`；P4.2 比赛详情 `1967:198`。P4.1 定义“来源、更新时间、受影响指标、前后分数和变化值”；P4.2 定义“对手、时间、场地、比分、个人比赛事件与教练点评”。
- Coach V7：C11 测评任务 `1973:2`；C12 项目选择 `1973:91`；C12.1 全队批量录入 `1973:131`；C14 能力评估 `1973:181`；C13 学员雷达 `1973:302`。C12.1 是任务→项目→全队连续录入的当前设计入口，固定保存／下一项目动作必须避开底部安全区。
- 写入后 Figma MCP 已回读各节点原生截图；这些是在线稿证据，不等同于小程序运行态视觉验收。

## 2026-09-03 C14 V7 层级清理

- 在线 Figma `zZ6wKyOHKcO4UYXDd9jGwv / 1973:181` 已隐藏旧版单人雷达、旧教练 TabBar 和旧操作提示层；这是非破坏性可见性调整，V7 球队能力概览层保留不动。
- 清理后重新读取 C14 截图，原生尺寸为 `375×812`，当前可见结构为浅粉顶栏、球队上下文、深色球队能力概览和两列学员卡。该截图是 Figma 设计证据，不等同于小程序运行态验收。

## 2026-09-04 C14 学员头像统一品牌色

- 用户确认能力评估的学员头像不应按首位/其余成员使用不同颜色。在线 C14 V7 `1973:181` 中原有的五个蓝色头像节点 `1976:25`、`1976:31`、`1976:37`、`1976:43`、`1976:49` 已统一为天才体育正红 `#A80F1B`；首位头像保持同色。
- Figma 重新截图回读为原生 `375×812`，当前可见六张学员卡头像均为正红色。此为在线稿更新证据，运行态截图另行验收。
