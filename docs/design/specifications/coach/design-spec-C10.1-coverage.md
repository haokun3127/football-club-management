...
- 画板中的“已覆盖 3 项”是示例值。小程序须从真实 `coach/training-coverage` 响应中的已覆盖维度计算计数，不写入示例数值。
- “确认”仅表示结束当前只读预览并返回上一页；当前没有覆盖预览的保存接口，因此不得新增伪造写请求、成功态或 API 响应。

## 真实运行态复核（2026-08-28）

- 在线稿已通过 Figma MCP 重新读取：`zZ6wKyOHKcO4UYXDd9jGwv / 93:983`，输出为 `375×812`。
- WeChatIDE MCP 真实教练会话截图：`tmp/goal-c10-1-coverage-live.png`，严格 `375×812`。顶栏、学员覆盖标题、学员卡、能力进度条、底部确认栏和教练 TabBar 均存在，模拟器 console 未命中错误、异常或路由变量错误。
- 运行态真实接口返回 10 个能力维度并按内容滚动，在线稿仅展示 3 个示例维度；“已覆盖 8 项”和各维度进度均来自真实 `coach/training-coverage` 响应，不能替换为 Figma 的示例姓名、数量或进度。
- 本次只补充运行证据，没有新增覆盖保存接口、伪数据、API 响应或 Figma 写回。
