...
- 在线唯一基准：`zZ6wKyOHKcO4UYXDd9jGwv / 93:952`。该节点已同步为三层指标训练内容选择器；以下早期单列清单明细仅保留作历史参考。
- 页面结构：粉色返回顶栏、球队/图谱上下文、搜索框、横向一级能力、左侧二级指标、右侧按三级指标分组的两列训练动作卡、固定“临时超级组 / 选择”栏和教练端 TabBar。
- 动作卡只展示中文名称、训练剂量、预计时长和所属指标；不展示圆圈图标，也不展示“查看动作要点”入口。
- 真正的训练项目、指标层级、选择状态与汇总必须读取现有训练目录和保存接口；Figma 中的内容仅为布局示例。
- 2026-08-31 复验：在线 Figma 截图与微信开发者工具 MCP 的真实 `375×812` 运行图均已取得；证据目录固定为系统临时目录 `cq-talent-visual-evidence`。
