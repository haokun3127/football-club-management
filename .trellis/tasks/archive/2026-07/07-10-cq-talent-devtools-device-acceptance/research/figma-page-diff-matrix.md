...
| `pages/coach/tactical-board/index` | C7 旧稿 `93:877`、代码对齐 `233:2` | 已适配重构 | 旧稿使用深色球场、固定 11v11，并含首版排除的绘制/分享/录像 | 已实现绿色竖屏球场、真实比赛名单/替补池、3 套阵型、拖拽相对坐标、SQLite 保存/重置、赛后只读和教练权限 | P0 完成 |
...
## 9. UI-3 实施回写（2026-07-11）

`06 Coach Generated` 保留原 C 系列探索画板，并新增 7 个 375×812 可编辑代码对齐画板：

| 代码路由 | Figma 代码对齐画板 | 节点 | 最终处理 |
| --- | --- | --- | --- |
| 教练任务工作台 | `CODE / C1 Coach Task Workbench` | `231:2` | 今日/本周范围、统计、筛选和每活动唯一下一步 |
| 活动工作台 | `CODE / C2 Activity Workbench` | `231:27` | 深色活动头、记录完善度、按类型显示动作、名单摘要 |
| 销课确认 | `CODE / C5 Lesson Confirmation` | `231:48` | 默认销课、例外原因、返还/补扣和固定确认栏 |
| 比赛录入 | `CODE / C6 Match Entry` | `231:65` | 比分 hero、capabilities 事件、无助攻默认和固定保存栏 |
| 训练内容 | `CODE / C10 Training Content` | `231:87` | 明确当前活动、已选项目、折叠搜索项目库和回填保存 |
| 整队评测 | `CODE / C12 Team Assessment` | `231:108` | 8 组 62 项、按项目录 25 人、缺测/草稿和固定提交栏 |
| 教练我的 | `CODE / C16 Coach Profile` | `231:137` | 真实身份、负责球队与权限范围，删除固定统计和假入口 |
| 比赛战术板 | `CODE / C7 Tactical Board MVP` | `233:2` | 绿色球场、真实 11 首发＋14 替补、阵型切换、拖拽比例坐标、保存/重置和只读边界 |

代码渲染与 Figma 画板均不恢复旧 C12/C15 的“按学员连续填 62 项”结构，也不显示公式、表格行号、内部版本 ID、假计时或固定业绩统计。
