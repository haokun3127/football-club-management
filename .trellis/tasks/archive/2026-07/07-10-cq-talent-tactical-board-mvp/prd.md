...
- [x] formations API 返回至少三套合法模板和 11 个 `0..1` 位置。
- [x] 有权限比赛教练可读取、拖拽、保存、重进恢复和重置；API 重启后数据仍存在。
- [x] 25 人 roster 中 11 人可按阵型上场，其余在替补池；重复/越界/非名单学生写入被拒绝。
- [x] 家长、无权限教练、非比赛活动不能编辑；完赛/取消比赛只读。
- [x] 拖拽不误滚页面，小屏/常规屏/大屏还原位置一致。
- [x] 页面不包含画线、录像、分享、多协同或家长入口。
- [x] Figma C7 与最终 MVP 结构同步，DevTools 截图通过。
- [x] domain/API/小程序类型检查、repository/contract/纯函数测试和 smoke 通过。
