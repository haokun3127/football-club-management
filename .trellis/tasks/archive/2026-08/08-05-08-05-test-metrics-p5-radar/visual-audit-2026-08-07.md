...
2. **渲染层整体冻结（2026-08-07 午后确认）**：跨不同已校验路由的捕获 sha256 完全相同（a47649be 连续 4 张、横跨 radar/growth 两路由），且画面是错误合成——radar 页原生 canvas 悬浮在成长页陈旧 webview 帧上（用户感知为"点击详情只浮出一个雷达图"）。**根因已定位（2026-08-07 傍晚）：工作区未提交的 libVersion 降级 3.17.0→3.16.2。改回 3.17.0 后跨路由捕获哈希恢复正常（growth 0148df87 / radar e491fdd6），radar 页完整渲染（vision 逐项核实：导航/chips/六维 Hero/综合评分 68/维度详情/tab 齐全，无悬浮）。** 判据：跨路由同哈希 = 渲染层冻结。此状态下的视觉异常是合成器幻影，不得当作页面缺陷改码。注意：3.16.2 下该冻结可跨 DevTools 重启复现，并非偶发。
3. 视觉分析后备通道：当会话内无 vision_analyze 工具时，`hermes chat -q "<定向问题>" --image <png>` 可完成图片分析（约 2-4 分钟/张，需 auxiliary.vision 配置在位）。本次用它确认了冻结帧内容（成长页+悬浮雷达）、成长页渲染完整性（标签/网格线齐全）与 3.17.0 下 radar 页完整渲染。
