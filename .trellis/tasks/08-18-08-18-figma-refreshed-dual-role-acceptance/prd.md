...
- [ ] 每个家长端与教练端画板均有当轮在线 Figma 读取记录，且确认路由、节点 ID、结构与数据豁免。
- [ ] 所有发现的实现差异均已按最小批次修复，或被明确列为真实 API 数据/平台系统壳层的非 UI 缺陷。
- [ ] 每一批代码改动通过相关小程序测试、TypeScript 检查与 `git diff --check`；最终运行全仓门禁。
- [ ] 可截图的页面具有当前 `375×812` 运行态对照证据；无法取证的页面不伪造通过结论。
- [ ] 每批次代码与任务/设计记录均单独提交，`docs/current/progress.md` 和相关 design spec 如实记录状态、证据与尚存限制。
