...
- [ ] CLI `islogin` 返回 true，小程序 typecheck 通过。
- [ ] DevTools `open` 和 `preview` 成功，记录测试 AppID、包体和二维码生成结果。
- [ ] 全新临时数据库上的 app-client smoke 19/19 通过。
- [ ] DevTools 模拟器能启动小程序并访问本地 API；页面不是白屏或仅静态壳。
- [ ] 家长端能看到 2 名真实家庭孩子，Tab、家庭日程、差异化详情、雷达切换/下钻和孩子档案可用，不泄露其他孩子。
- [ ] 教练端能进入任务工作台和关键业务入口，页面操作与活动类型/权限一致。
- [ ] 连续切换顶层 Tab 不堆积页面栈，页面无明显技术文案或阻断性报错。
- [ ] 真机扫码结果、机型、微信版本、基础库版本和人工结论被记录；未由手机实际验证的项目保持未完成状态。
- [ ] Figma 与当前实现完成页面级差异清单，明确“直接还原、适配新结构、从设计系统派生”三类处理方式；未完成对齐前不宣称最终 UI 验收通过。
- [ ] 当前 15 个注册页面全部完成 Figma 风格对齐并经过逐页渲染检查，不保留明显旧壳页面。
- [ ] 战术板能从有权限比赛活动进入，使用真实名单与阵型模板，拖动后保存 `x/y: 0..1`，重进保持位置；重置、屏幕适配、赛后只读和无权限隐藏通过验证。
- [ ] 战术板不包含首版明确排除的画线、录像、多人协作和家长入口。
- [ ] 完整 Figma 文件中与当前注册路由和战术板对应的页面已同步最终信息结构，DevTools 截图与 Figma 对比没有结构性冲突。
