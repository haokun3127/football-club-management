...
- [x] Compile `pages/coach/schedule/index.wxml` and `pages/coach/schedule/index.wxss` through the available WeChatIDE runtime path; the page mounted and rendered without WXML/runtime errors during the fresh MCP capture.
- [x] Refresh/open the route in a real coach session, capture fresh `375x812` evidence, and compare top and bottom sections with the online Figma screenshot.
...
- [x] Stage only C1 page files, C1 tests/spec/progress, and this task's planning artifacts; commit and push to `origin/dev`.
