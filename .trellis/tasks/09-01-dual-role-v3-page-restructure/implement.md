...
17.5. [~] C15 V6 当前设计为 `1623:2`（`375×812`、`Pending Code Verification`）：复用 C14 已验收的顶栏、训练球队上下文和全员头像选择结构；中间雷达已隐藏并替换为三张“原始值 + 标准分”指标卡（30米冲刺、定点射门、平板支撑）。空容器 `1621:2` 已由本轮创建者安全替换，旧 C15 `93:1132` 保留不动；尚未进行代码实现或实机验收。
