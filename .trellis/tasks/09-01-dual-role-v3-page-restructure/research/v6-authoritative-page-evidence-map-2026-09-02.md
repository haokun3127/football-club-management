...
| 页面 | 当前 Figma 节点 | 运行路由 | 运行证据 | 状态 |
| --- | --- | --- | --- | --- |
| C1 全部球队日程 | `1610:1323` | `/pages/coach/schedule/index` | `wechatide-simulator-screenshot-1788313532147-80prf1.png`（系统临时目录，375×812） | 结构、全队范围、周切换箭头、空态和 TabBar 已通过；当前日期真实无日程 |
| C2 训练工作台 | `1610:1462` | `/pages/coach/event/index?id=<trainingActivityId>` | `wechatide-simulator-screenshot-1788313548282-r5fftx.png`（系统临时目录，375×812） | 工作台、二态点名、训练内容、评测入口和真实学员数据已通过 |
...
| C6 比赛记录 | `1610:1692` | `/pages/coach/match/index?id=<matchActivityId>` | `wechatide-simulator-screenshot-1788313572441-rm48if.png`（系统临时目录，375×812） | 比分、事件时间线、页内草稿提示和入口已通过 |
...
| C7 战术板 | `1610:1778` | `/pages/coach/tactical-board/index?eventId=<matchActivityId>` | `wechatide-simulator-screenshot-1788313584617-6eju3b.png`（系统临时目录，375×812） | 满屏球场、球员区、阵型和保存区已通过 |
| C8 训练管理 | `1610:1843` | `/pages/coach/training/index` | `wechatide-simulator-screenshot-1788313603810-5qm7y9.png`（系统临时目录，375×812） | 训练球队上下文、切换入口和课程管理结构已通过 |
| C8.1 选择训练球队 | `1610:1950` | `/pages/coach/team-selector/index` | `wechatide-simulator-screenshot-1788313615953-8bjr33.png`（系统临时目录，375×812） | 全屏选队、后台分配范围和选中态已通过 |
