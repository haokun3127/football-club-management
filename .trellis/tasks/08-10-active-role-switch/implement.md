...
5. **In-app switching controls (TDD).** Add parent-account and coach-me visibility/routing tests. Show a control only if the opposite role is in `availableRoles`; call the server endpoint, replace the stored session with its complete response, then relaunch the selected home. Keep coach-account untouched.
