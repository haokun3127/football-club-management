...
**三条候选路径，需要用户先裁决，别自己决定：**

1. 受控 INSERT 进生产库。写前必须 `VACUUM INTO` 备份，写后必须 `docker restart cq-talent-api`（见第五节铁律）。
2. 只在本地带 `FCM_CQ_TALENT_ACCEPTANCE_SEED=1` 跑 API，小程序基址指本地。零外部影响，适合先做视觉验收。
3. 不灌数，按设计稿把空态引导做完整。
