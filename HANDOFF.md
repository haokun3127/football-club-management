...
## 六、当前安全演示数据（2026-08-18）

七个运行时私有手机号槽位均为 active 的 parent + coach 双角色；每个槽位保持两个家长可见孩子和一个八人教练 roster，含日程、出勤、课时、八维指标/评估、比赛和已保存战术板数据。发布后受控 dry-run 返回 `already_present`，且服务端 BFF 回读验证了上述 parent/coach scope。运行中的 API release 为 `30d2869`；健康检查通过不代表真机微信授权或视觉验收已经完成。

以下为 2026-08-12 三槽位补数的历史说明，保留仅作背景：

三个测试账号，都是家长+教练双角色。手机号仅从私有运行时配置或向操作员确认，不写入交接文档。每个账号：
