...
describe("coach schedule home", () => {
  ...
  it("keeps C1 week navigation visible and separately tappable", () => {
    ...
    expect(stylesheet).toMatch(/\.c1-dates__arrow--next\s*\{[^}]*top:\s*48rpx[^}]*right:\s*0[^}]*width:\s*32rpx[^}]*height:\s*32rpx/s);
    expect(stylesheet).toMatch(/\.c1-dates__arrow--previous\s+image\s*\{[^}]*width:\s*48rpx[^}]*height:\s*48rpx/s);
    expect(stylesheet).toMatch(/\.c1-dates__arrow--next\s+image\s*\{[^}]*width:\s*32rpx[^}]*height:\s*32rpx/s);
    ...
    expect(stylesheet).toMatch(/\.c1-dates__expand\s*\{[^}]*right:\s*24rpx[^}]*width:\s*60rpx[^}]*height:\s*32rpx[^}]*border-radius:\s*16rpx/s);
